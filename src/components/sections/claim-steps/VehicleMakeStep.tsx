...
import { vehicleMakes } from '@/data/carClaimSteps';
...
export function VehicleMakeStep({
    ...
    const [subStep, setSubStep] = useState<'chips' | 'search' | 'confirmed'>(
        data.vehicleMake ? 'confirmed' : 'chips'
    );
    ...
    const handleSearchSelect = () => {
        if (searchQuery.trim()) {
            updateData({ vehicleMake: searchQuery.trim() });
            setSubStep('confirmed');
        }
    };

    const filteredMakes = vehicleMakes.filter(
        (m) => m !== 'Other brand' && m.toLowerCase().includes(searchQuery.toLowerCase())
    );
    ...
    return (
        <div className={cn('flex flex-col items-center w-full', className)}>
            ...
            {/* Sub-step: Chips */}
            {subStep === 'chips' && (
                <div className="flex flex-wrap justify-center gap-[16px] md:gap-[24px] max-w-[673px]">
                    {vehicleMakes.map((make) => (
                        <button
                            key={make}
                            onClick={() => handleSelectMake(make)}
                            className="px-[24px] py-[14px] border border-[#c9c9c9] rounded-[8px] font-['Source_Sans_Pro',sans-serif] text-[16px] leading-[24px] text-[#1a1d21] hover:border-[#00008f] hover:text-[#00008f] transition-colors whitespace-nowrap"
                        >
                            {make}
                        </button>
                    ))}
                ...
                <div className="w-full max-w-[372px]">
                    <div className="flex items-center border border-[#c9c9c9] rounded-[8px] px-[16px] py-[12px] gap-[8px]">
                        <input
                            ...
                            autoFocus
                        />
                        <Search size={20} className="text-[#434956] shrink-0" />
                    </div>

                    {/* Filtered suggestions */}
                    {searchQuery && filteredMakes.length > 0 && (
                        <div className="mt-[8px] border border-[#e0e0e0] rounded-[8px] overflow-hidden">
                            {filteredMakes.map((make) => (
                                <button
                                    ...
                                    className="w-full px-[16px] py-[10px] text-left font-['Source_Sans_Pro',sans-serif] text-[16px] text-[#1a1d21] hover:bg-[#f5f5f5] transition-colors"
                                >
                                    {make}
                                </button>
                            ))}
            ...
            {/* Sub-step: Confirmed */}
            {subStep === 'confirmed' && (
                <div className="flex flex-col items-center gap-[32px] w-full max-w-[372px]">
                    <div className="flex items-center justify-between w-full border border-[#c9c9c9] rounded-[8px] px-[16px] py-[12px]">
                        <span className="font-['Source_Sans_Pro',sans-serif] text-[16px] leading-[24px] text-[#1a1d21]">
                            {data.vehicleMake}
                        </span>
