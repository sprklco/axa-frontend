...
import type { HighlightBanner } from "./HighlightBannersSection";

export interface HighlightBannerListSectionProps {
  banners: readonly HighlightBanner[];
  className?: string;
}

function HighlightBannerCard({ banner }: { banner: HighlightBanner }) {
  return (
      ...
      <div className="flex flex-1 flex-col justify-between gap-6">
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            <div className="flex h-[32px] w-[38px] items-center justify-center">
              <svg
          ...
          <p className="font-source-sans max-w-[489px] text-[18px] leading-[26px] text-[#1a1d21]">
            {banner.lead}
          </p>

          <ul className="font-source-sans list-disc space-y-1 pl-7 text-[18px] leading-[26px] text-[#1a1d21]">
...
export function HighlightBannerListSection({
  banners,
  className,
}: HighlightBannerListSectionProps) {
  if (!banners.length) {
    return null;
  }
  ...
  return (
    <section className={cn("bg-[#f7f7f8] py-12 md:py-16 lg:py-20", className)}>
      <Container className="flex flex-col gap-8 lg:gap-10">
        {banners.map((banner) => (
          <HighlightBannerCard key={banner.id} banner={banner} />
        ))}
      </Container>
    </section>
  );
